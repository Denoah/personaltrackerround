  ...
  implements Interceptor
{
  ...
  private final Request buildRedirectRequest(Response paramResponse, String paramString)
  {
    ...
    if (localObject != null)
    {
      ...
      {
        if ((!Intrinsics.areEqual(localHttpUrl.scheme(), paramResponse.request().url().scheme())) && (!this.client.followSslRedirects())) {
          return null;
        }
        localObject = paramResponse.request().newBuilder();
        if (HttpMethod.permitsRequestBody(paramString))
        {
          ...
          {
            ((Request.Builder)localObject).method("GET", null);
          }
          else
          {
            if (bool) {
            ...
            ((Request.Builder)localObject).method(paramString, localRequestBody);
          }
          if (!bool)
          {
            ((Request.Builder)localObject).removeHeader("Transfer-Encoding");
            ((Request.Builder)localObject).removeHeader("Content-Length");
            ((Request.Builder)localObject).removeHeader("Content-Type");
          }
        }
        if (!Util.canReuseConnectionFor(paramResponse.request().url(), localHttpUrl)) {
          ((Request.Builder)localObject).removeHeader("Authorization");
        }
        return ((Request.Builder)localObject).url(localHttpUrl).build();
      }
    }
    return null;
  }
      ...
      try
      {
        ...
        if (!bool) {
          try
          {
            ...
            paramChain = new java/lang/StringBuilder;
            paramChain.<init>();
            paramChain.append("Too many follow-up requests: ");
            paramChain.append(i);
            ((ProtocolException)localObject).<init>(paramChain.toString());
