  ...
  extends RequestBody
{
  ...
  public MultipartBody(ByteString paramByteString, MediaType paramMediaType, List<Part> paramList)
  {
    ...
    paramMediaType = MediaType.Companion;
    paramByteString = new StringBuilder();
    paramByteString.append(this.type);
    paramByteString.append("; boundary=");
    paramByteString.append(boundary());
    this.contentType = paramMediaType.get(paramByteString.toString());
    ...
    throws IOException
  {
    ...
    for (int j = 0; j < i; j++)
    {
      ...
      if (localObject2 != null)
      {
        ...
        for (int m = 0; m < k; m++) {
          paramBufferedSink.writeUtf8(((Headers)localObject2).name(m)).write(COLONSPACE).writeUtf8(((Headers)localObject2).value(m)).write(CRLF);
        }
      }
      localObject2 = ((RequestBody)localObject1).contentType();
      if (localObject2 != null) {
        paramBufferedSink.writeUtf8("Content-Type: ").writeUtf8(((MediaType)localObject2).toString()).write(CRLF);
      }
      ...
      if (paramBoolean) {
        ...
        ((RequestBody)localObject1).writeTo(paramBufferedSink);
      }
      paramBufferedSink.write(CRLF);
    }
    if (paramBufferedSink == null) {
    ...
    throws IOException
  {
    ...
    if (l1 == -1L)
    {
      l2 = writeOrCountBytes(null, true);
