  ...
  implements Closeable
{
  ...
  {
    CacheControl localCacheControl1 = this.lazyCacheControl;
    CacheControl localCacheControl2 = localCacheControl1;
    if (localCacheControl1 == null)
    {
      localCacheControl2 = CacheControl.Companion.parse(this.headers);
      this.lazyCacheControl = localCacheControl2;
    }
    ...
    throws IOException
  {
    ...
    BufferedSource localBufferedSource = ((ResponseBody)localObject).source().peek();
    localObject = new Buffer();
    localBufferedSource.request(paramLong);
    ((Buffer)localObject).write((Source)localBufferedSource, Math.min(paramLong, localBufferedSource.getBuffer().size()));
    return ResponseBody.Companion.create((BufferedSource)localObject, this.body.contentType(), ((Buffer)localObject).size());
  }
