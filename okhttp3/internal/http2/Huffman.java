...
public final class Huffman
{
  ...
  static
  {
    ...
    for (int j = 0; j < i; j++) {
      localHuffman.addCode(j, CODES[j], CODE_BIT_COUNTS[j]);
    }
  }
  ...
  {
    Node localNode1 = new Node(paramInt1, paramInt3);
    ...
    while (paramInt3 > 8)
    {
      ...
      Node localNode2 = arrayOfNode[paramInt1];
    ...
    ArraysKt.fill((Object[])localObject, localNode1, paramInt2, (1 << paramInt1) + paramInt2);
  }
  ...
  public final void decode(BufferedSource paramBufferedSource, long paramLong, BufferedSink paramBufferedSink)
  {
    ...
    for (;;)
    {
      ...
      i = i << 8 | Util.and(paramBufferedSource.readByte(), 255);
      j += 8;
      while (j >= 8)
      {
        k = j - 8;
        ...
        localObject1 = localObject1[(i >>> k & 0xFF)];
        if (localObject1 == null) {
          Intrinsics.throwNpe();
        }
        if (((Node)localObject1).getChildren() == null)
        {
          ...
          j -= ((Node)localObject1).getTerminalBitCount();
          localObject1 = root;
        }
        else
        {
    ...
    while (k > 0)
    {
      ...
      paramBufferedSource = paramBufferedSource[(i << 8 - k & 0xFF)];
      if (paramBufferedSource == null) {
        Intrinsics.throwNpe();
      }
      if ((paramBufferedSource.getChildren() != null) || (paramBufferedSource.getTerminalBitCount() > k)) {
        break;
      }
    ...
    throws IOException
  {
    ...
    while (j < i)
    {
      int m = Util.and(paramByteString.getByte(j), 255);
      int n = CODES[m];
      m = CODE_BIT_COUNTS[m];
      l = l << m | n;
      k += m;
      ...
      {
        k -= 8;
        paramBufferedSink.writeByte((int)(l >> k));
      }
      j++;
    }
    if (k > 0) {
      paramBufferedSink.writeByte((int)(l << 8 - k | 255L >>> k));
    }
  }
  ...
  public final int encodedLength(ByteString paramByteString)
  {
    ...
    for (int j = 0; j < i; j++)
    {
      int k = Util.and(paramByteString.getByte(j), 255);
      l += CODE_BIT_COUNTS[k];
    }
    return (int)(l + 7 >> 3);
  }
