...
public final class ObservableInternalHelper
{
    ...
    implements Function<T, ObservableSource<U>>
  {
    ...
    public ObservableSource<U> apply(T paramT)
      throws Exception
    {
      return new ObservableFromIterable((Iterable)ObjectHelper.requireNonNull(this.mapper.apply(paramT), "The mapper returned a null Iterable"));
    }
  }
    ...
    implements Function<T, ObservableSource<R>>
  {
    ...
    public ObservableSource<R> apply(T paramT)
      throws Exception
    {
      return new ObservableMap((ObservableSource)ObjectHelper.requireNonNull(this.mapper.apply(paramT), "The mapper returned a null ObservableSource"), new ObservableInternalHelper.FlatMapWithCombinerInner(this.combiner, paramT));
    }
  }
    ...
    implements Function<T, ObservableSource<T>>
  {
    ...
    public ObservableSource<T> apply(T paramT)
      throws Exception
    {
      return new ObservableTake((ObservableSource)ObjectHelper.requireNonNull(this.itemDelay.apply(paramT), "The itemDelay returned a null ObservableSource"), 1L).map(Functions.justFunction(paramT)).defaultIfEmpty(paramT);
    }
  }
    ...
    implements Function<Observable<T>, ObservableSource<R>>
  {
    ...
    public ObservableSource<R> apply(Observable<T> paramObservable)
      throws Exception
    {
      return Observable.wrap((ObservableSource)ObjectHelper.requireNonNull(this.selector.apply(paramObservable), "The selector returned a null ObservableSource")).observeOn(this.scheduler);
    }
  }
