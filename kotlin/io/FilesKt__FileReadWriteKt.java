  ...
  public static final byte[] readBytes(File paramFile)
  {
    ...
    try
    {
      ...
      if (l <= Integer.MAX_VALUE)
      {
        ...
        while (j > 0)
        {
          int m = localFileInputStream.read((byte[])localObject2, k, j);
          if (m < 0) {
            break;
          }
          j -= m;
        ...
        if (j > 0)
        {
          paramFile = Arrays.copyOf((byte[])localObject2, k);
        ...
        else
        {
          ...
          {
            ExposingBufferByteArrayOutputStream localExposingBufferByteArrayOutputStream = new kotlin/io/ExposingBufferByteArrayOutputStream;
            localExposingBufferByteArrayOutputStream.<init>(8193);
            localExposingBufferByteArrayOutputStream.write(k);
            ...
            k = localExposingBufferByteArrayOutputStream.size() + i;
            if (k < 0) {
              break label238;
            }
            paramFile = localExposingBufferByteArrayOutputStream.getBuffer();
            localObject2 = Arrays.copyOf((byte[])localObject2, k);
            Intrinsics.checkExpressionValueIsNotNull(localObject2, "java.util.Arrays.copyOf(this, newSize)");
            paramFile = ArraysKt.copyInto(paramFile, (byte[])localObject2, i, 0, localExposingBufferByteArrayOutputStream.size());
          }
        }
        CloseableKt.closeFinally(localCloseable, (Throwable)localObject1);
        return paramFile;
        label238:
        localObject2 = new java/lang/OutOfMemoryError;
        ...
        ((OutOfMemoryError)localObject2).<init>(((StringBuilder)localObject1).toString());
      ...
      ((StringBuilder)localObject2).append(" is too big (");
      ((StringBuilder)localObject2).append(l);
      ((StringBuilder)localObject2).append(" bytes) to fit in memory.");
      ((OutOfMemoryError)localObject1).<init>(((StringBuilder)localObject2).toString());
