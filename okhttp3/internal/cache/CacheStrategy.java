  ...
  public static final class Factory
  {
    ...
    public Factory(long paramLong, Request paramRequest, Response paramResponse)
    {
      ...
      if (paramResponse != null)
      {
        ...
        Headers localHeaders = this.cacheResponse.headers();
        int i = 0;
        int j = localHeaders.size();
        while (i < j)
        {
          paramRequest = localHeaders.name(i);
          paramResponse = localHeaders.value(i);
          if (StringsKt.equals(paramRequest, "Date", true))
          {
            this.servedDate = DatesKt.toHttpDateOrNull(paramResponse);
            this.servedDateString = paramResponse;
          }
          else if (StringsKt.equals(paramRequest, "Expires", true))
          {
            this.expires = DatesKt.toHttpDateOrNull(paramResponse);
          }
          else if (StringsKt.equals(paramRequest, "Last-Modified", true))
          {
            this.lastModified = DatesKt.toHttpDateOrNull(paramResponse);
            this.lastModifiedString = paramResponse;
          }
          else if (StringsKt.equals(paramRequest, "ETag", true))
          {
            this.etag = paramResponse;
          }
          else if (StringsKt.equals(paramRequest, "Age", true))
          {
    ...
    {
      CacheStrategy localCacheStrategy1 = computeCandidate();
      CacheStrategy localCacheStrategy2 = localCacheStrategy1;
      if (localCacheStrategy1.getNetworkRequest() != null)
      {
