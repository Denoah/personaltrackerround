...
public final class FlowableInternalHelper
{
    ...
    implements Function<T, Publisher<U>>
  {
    ...
    public Publisher<U> apply(T paramT)
      throws Exception
    {
      return new FlowableFromIterable((Iterable)ObjectHelper.requireNonNull(this.mapper.apply(paramT), "The mapper returned a null Iterable"));
    }
  }
    ...
    implements Function<U, R>
  {
    ...
    public R apply(U paramU)
      throws Exception
    {
      return this.combiner.apply(this.t, paramU);
    }
  }
    ...
    implements Function<T, Publisher<R>>
  {
    ...
    public Publisher<R> apply(T paramT)
      throws Exception
    {
      return new FlowableMapPublisher((Publisher)ObjectHelper.requireNonNull(this.mapper.apply(paramT), "The mapper returned a null Publisher"), new FlowableInternalHelper.FlatMapWithCombinerInner(this.combiner, paramT));
    }
  }
    ...
    implements Function<T, Publisher<T>>
  {
    ...
    public Publisher<T> apply(T paramT)
      throws Exception
    {
      return new FlowableTakePublisher((Publisher)ObjectHelper.requireNonNull(this.itemDelay.apply(paramT), "The itemDelay returned a null Publisher"), 1L).map(Functions.justFunction(paramT)).defaultIfEmpty(paramT);
    }
  }
    ...
    implements Function<Flowable<T>, Publisher<R>>
  {
    ...
    public Publisher<R> apply(Flowable<T> paramFlowable)
      throws Exception
    {
      return Flowable.fromPublisher((Publisher)ObjectHelper.requireNonNull(this.selector.apply(paramFlowable), "The selector returned a null Publisher")).observeOn(this.scheduler);
    }
  }
    ...
    implements Function<List<Publisher<? extends T>>, Publisher<? extends R>>
  {
    ...
    {
      return Flowable.zipIterable(paramList, this.zipper, false, Flowable.bufferSize());
    }
  }
}
