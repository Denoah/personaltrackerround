...
public final class RouteSelector
{
    ...
    throws IOException
  {
    ...
    {
      Object localObject1 = paramProxy.address();
      if ((localObject1 instanceof InetSocketAddress))
      {
        localObject2 = Companion;
        localObject1 = (InetSocketAddress)localObject1;
        localObject2 = ((Companion)localObject2).getSocketHost((InetSocketAddress)localObject1);
        i = ((InetSocketAddress)localObject1).getPort();
      }
      else
      {
        paramProxy = new StringBuilder();
        paramProxy.append("Proxy.address() is not an InetSocketAddress: ");
        paramProxy.append(localObject1.getClass());
        throw ((Throwable)new IllegalArgumentException(paramProxy.toString().toString()));
      }
    }
    else
    {
      localObject2 = this.address.url().host();
    ...
    if ((1 <= i) && (65535 >= i))
    {
      ...
      {
        ((Collection)localList).add(InetSocketAddress.createUnresolved((String)localObject2, i));
      }
      else
      {
        this.eventListener.dnsStart(this.call, (String)localObject2);
        paramProxy = this.address.dns().lookup((String)localObject2);
        if (paramProxy.isEmpty()) {
          break label290;
        }
        this.eventListener.dnsEnd(this.call, (String)localObject2, paramProxy);
        paramProxy = paramProxy.iterator();
        while (paramProxy.hasNext())
        {
          localObject2 = (InetAddress)paramProxy.next();
          ((Collection)localList).add(new InetSocketAddress((InetAddress)localObject2, i));
        }
      }
      return;
      label290:
      ...
      paramProxy.append((String)localObject2);
