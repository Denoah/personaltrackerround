  ...
  implements ExecutionListener
{
  ...
  public boolean add(Intent paramIntent, int paramInt)
  {
    ...
    String str = TAG;
    int i = 0;
    ((Logger)localObject).debug(str, String.format("Adding command %s (%s)", new Object[] { paramIntent, Integer.valueOf(paramInt) }), new Throwable[0]);
    assertMainThread();
    ...
    if (("ACTION_CONSTRAINTS_CHANGED".equals(localObject)) && (hasIntentWithAction("ACTION_CONSTRAINTS_CHANGED"))) {
      return false;
    }
    ...
    synchronized (this.mIntents)
    {
      if (this.mCurrentIntent != null)
      {
        ...
        else
        {
          ...
          ((IllegalStateException)localObject1).<init>("Dequeue-d command is not the first.");
          throw ((Throwable)localObject1);
        }
      }
      Object localObject1 = this.mTaskExecutor.getBackgroundExecutor();
      if ((!this.mCommandHandler.hasPendingCommands()) && (this.mIntents.isEmpty()) && (!((SerialExecutor)localObject1).hasPendingTasks()))
      {
