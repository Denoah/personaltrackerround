  ...
  implements SettingsJsonTransform
{
    ...
    throws JSONException
  {
    ...
    if ((paramJSONObject.has("icon")) && (paramJSONObject.getJSONObject("icon").has("hash"))) {
      paramJSONObject = buildIconDataFrom(paramJSONObject.getJSONObject("icon"));
    } else {
      paramJSONObject = null;
    }
    return new AppSettingsData(str1, str2, str3, str4, str5, bool, paramJSONObject);
  }
  ...
  {
    return new FeaturesSettingsData(paramJSONObject.optBoolean("prompt_enabled", false), paramJSONObject.optBoolean("collect_logged_exceptions", true), paramJSONObject.optBoolean("collect_reports", true), paramJSONObject.optBoolean("collect_analytics", false), paramJSONObject.optBoolean("firebase_crashlytics_enabled", false));
  }
    ...
    throws JSONException
  {
    if (paramJSONObject.has("expires_at")) {
      ...
      paramLong = paramCurrentTimeProvider.getCurrentTimeMillis() + paramLong * 1000L;
    }
    return paramLong;
  }
  ...
  private JSONObject toAppJson(AppSettingsData paramAppSettingsData)
    throws JSONException
  {
    JSONObject localJSONObject = new JSONObject().put("identifier", paramAppSettingsData.identifier).put("status", paramAppSettingsData.status).put("url", paramAppSettingsData.url).put("reports_url", paramAppSettingsData.reportsUrl).put("ndk_reports_url", paramAppSettingsData.ndkReportsUrl).put("update_required", paramAppSettingsData.updateRequired);
